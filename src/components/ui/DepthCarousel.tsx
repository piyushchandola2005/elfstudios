...
// Minimal types for the adapted DepthCarousel
...
const DepthCarousel = forwardRef<DepthCarouselRef, DepthCarouselProps>(({
  items = [],
  cardWidth = 400, // Adjusted default for forms
  cardHeight = 550, // Adjusted default for forms
  radius = 18,
  tint = '#05060a',
  depth = 220,
  spread = 90,
  ...
  autoplay = false,
  autoplayDelay = 3200,
  loop = false, // Changed default to false for a wizard/booking flow
  showControls = true,
  showIndicators = true,
  onChange,
  className = ''
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Prevent default scrolling only if the target is NOT inside a scrollable content area
      const target = e.target as HTMLElement;
      if (target.closest('.depth-carousel__content')) {
         // let the native scroll happen inside the card if needed
         return; 
      }
      ...
      // If not looping, prevent scrolling past bounds
      let nextPos = posRef.current + step;
      if (!cfg.loop) {
        nextPos = clamp(nextPos, 0, cfg.count - 1);
      }
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    // Check if dragging from inside a scrollable area or input
    const target = e.target as HTMLElement;
    if (target.closest('.depth-carousel__content') || target.tagName === 'INPUT' || target.tagName === 'BUTTON') {
      return; 
    }
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      let nextPos = drag.startPos - dx / stepPx;
      if (!cfg.loop) {
        nextPos = clamp(nextPos, -0.5, cfg.count - 0.5); // Allow slight rubber band
      }
  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      // Don't intercept arrow keys if typing in an input
      const target = e.target as HTMLElement;
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            {item.content ? (
              <div className="depth-carousel__content w-full h-full p-8 bg-[#1A1A1A] overflow-y-auto custom-scrollbar flex flex-col">
                {item.content}
              </div>
            ) : (
      ...
      {showControls && count > 1 && (
        <>
          <button
            ...
            style={{ opacity: (!loop && active === 0) ? 0.3 : 1, cursor: (!loop && active === 0) ? 'not-allowed' : 'pointer' }}
          >
          ...
          <button
            ...
            style={{ opacity: (!loop && active === count - 1) ? 0.3 : 1, cursor: (!loop && active === count - 1) ? 'not-allowed' : 'pointer' }}
          >
            <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
              <path
                d="M9 5l7 7-7 7"
