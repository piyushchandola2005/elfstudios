...
test("pricing follows attendee bands", () => {
  ...
  assert.equal(calculatePrice(6, 2).totalPaise, 120_000);
...
test("slot validation rejects duplicates and invalid hours", () => {
  assert.deepEqual(validateSlots(["11", "10"]), ["10", "11"]);
  assert.throws(() => validateSlots(["10", "10"]));
  assert.throws(() => validateSlots(["9"]));
});
